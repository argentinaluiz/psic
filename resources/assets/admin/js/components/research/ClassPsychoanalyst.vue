...
<script>
    ...
    export default {
        props: ['research'], //para passar o id da Classe
        computed: {
            psychoanalysts(){
                return store.state.classPsychoanalyst.psychoanalysts;
            }
        },
        mounted(){ //invoca a action classPsychoanalyst/query, que busca os dados através de uma requisição Ajax e alimenta os dados de psicanalistas para que seja listado no componente
            store.dispatch('classPsychoanalyst/query', this.research); //passo o módulo e depois a ação que desejo chamar
            ...
            let self = this;
                $("select[name=psychoanalysts]").on('select2:select', event => {
                    store.dispatch('classPsychoanalyst/store', {
                        psychoanalystId: event.params.data.id,
                        researchId: self.research
                    }).then(() => {
                        new PNotify({
                            title: 'Aviso',
